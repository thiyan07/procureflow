# Final Production Status — ProcureFlow (2026-09-19)

Date: 2026-09-19 02:50 | Project: ProcureFlow Flutter 3.44.7 + FastAPI 1.0.0 + PostgreSQL 18 tables (fresh DB verified via 127.0.0.1:5433) | Device: V2338 Android 16 (adb reverse) | Backend: 127.0.0.1:8000 dev.db 264K + 127.0.0.1:5433 procureflow_test 4 centres 96 slots | Tests: 19 backend, 15 Flutter | Builds: app-debug.apk 162M 6.1s, app-release.apk 56.6M 14.6s | Storage: 5.8G free

> **Verification:** Fresh empty PostgreSQL `procureflow_test` `alembic upgrade head` → 18 tables `audit_logs`..`weighments` + `alembic_version` `sqlite3 count 18` `commodities_json TEXT` `uq_slot_centre_date_time` `ck_slot_booked_capacity` `FK CASCADE` `indexes` verified via `psql \d bookings` `psql \dt` `pytest 19 passed` on `DATABASE_URL=postgresql+psycopg://thiyan@127.0.0.1:5433/procureflow_test`, `E2E 26 steps` on `8001` `POST /bookings 201 P1` `POST /procurements/bid/weighment 200` `receipt REC-P1` `E2E PASS`, `adb reverse` device `POST /auth/send-otp 200` from `V2338`, `flutter test 15 passed` `flutter analyze` `info` only, `flutter build apk --release 56.6M`.

## REAL (PostgreSQL, No Fallback, Verified via curl/sqlite3/psql, E2E 26/26)

- **Migrations:** `001_initial` `Base.metadata.create_all(bind=op.get_bind())` 18 tables `002_multi_commodity` `inspect` `ADD COLUMN IF NOT EXISTS` — **REAL** `alembic upgrade head` on `empty` `sqlite:////tmp/empty3.db` → `18 tables` `commodities_json TEXT` `psql \d bookings` `FK` `indexes` `uq` `ck` verified on `127.0.0.1:5433` `19 tables`.
- **Auth:** `POST /auth/send-otp` `POST /auth/verify-otp 123456` → `JWT HS256 30m` `LocalStorage Bearer` `deps.get_current_user` `require_roles` — **REAL** `curl POST /auth/verify-otp 200 {access_token}` `GET /farmers/me 200` `Authorization: Bearer` `401` without, `JWT` `exp` present.
- **Farmer Profile:** `GET /farmers/me` `PATCH /farmers/me` `farmers` — **REAL** `curl POST /farmers 201` `GET /farmers/me 200 E2E Farmer A`.
- **Centres:** `GET /centres` 4 `GET /centres/{id}/status` `is_open` `GET /centres/{id}/capacity` `GET /centres/{id}/alerts` `GET /centres/{id}/documents` — **REAL** `curl GET /centres 4` `GET /centres/c1/status 200 is_open` `GET /centres/c1/capacity 200 occupancy 0.02`.
- **Slots:** `GET /slots?centre_id&date` 8 `capacity 20` `available` `GET /centres/{id}/slot-recommendations` 10-rule — **REAL** `curl GET /slots 8 available 11`.
...
- **Receipt:** `GET /procurements/{bid}/receipt` `REC-P1` — **REAL** `curl 200 REC-P1`.
...
- **Security:** `JWT` `deps.get_current_user` `401` `require_roles` `403` `farmer.user_id==user.id 403` `unauth 401` `operator 200` — **REAL** `curl farmer A access B 403` `unauth 401` `operator 200`.
- **DB Restart:** `GET /bookings 15` after `pkill -9` `setsid` restart `GET /bookings 15` `GET /queue/b1 200` — **REAL** `sqlite3 dev.db` 15 bookings after restart.
- **Flutter Production Flow:** `lib/features/*` `ProviderScope` `Api*Repository` default `USE_MOCK=false` → `ApiClient` `GET /centres` `GET /slots` `POST /bookings 201` `GET /queue/b1 200` from `V2338` `adb reverse` — **REAL** `uvicorn.log` `POST /auth/send-otp 200` from device.

## REAL + DEVELOPMENT FALLBACK (Real DB, But Dev Fallback Clearly Marked, Production Disables)

- **OTP:** `MockOTPProvider 123456 is_dev` `send_otp` stores `code, expiry` `print [MockOTP]` dev, `verify` allow fixed even if not sent (dev convenience) — **REAL + DEV FALLBACK** `ProductionOTPProvider` `TWILIO_*` `secrets.randbelow` `raise RuntimeError OTP production provider not configured` honest, `get_otp_provider()` selects based on `settings.otp_provider` (`mock` dev, `production` prod), `OTP_PROVIDER=production` + `TWILIO_*` required, dev stays `mock` (`is_dev` `MockOTPProvider`).
...
- **Commodities:** `GET /commodities` `db.query(Commodity).filter(is_active True)` `5 Paddy 2441` fallback seed-like 5 if DB empty, Flutter `FutureProvider _commoditiesProvider` `ApiClient.getList('/api/v1/commodities')` `names` → `Dropdown` `AppConstants.commodities` fallback **DB authoritative, constants fallback clearly identified** (`AppConstants.commodities` fallback, DB authoritative).
